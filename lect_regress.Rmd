...
$$Y = f(X_1, X_2, \ldots , X_p) + ε = β_0 + β_1X_1 + β_2X_2 + \cdot + β_pX_p + ε$$
...
There are $p$ explanatory variables (predictors) ($j=1,\ldots, p$); $x_{ij}$ is the value of the explanatory
variable $X_j$ of the $i$th case:
...
Least Squareq** (**OLS**) provides estimators of regression function with
...
In a linear regression model $Y = β 0 + β_1 X_1 + . . . + β_p X_p + ε$
one can include **categorical**? (ie non-numeric) variables
as predictors.
...
So, one **can not** include $l_1, l_l$ variables in the model, but only $l_1, l_{l-1}$ (strictly speaking
all but one $l$ variable, regardless which one)

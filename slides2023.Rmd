...
Zmienna losowa: zmienna, która przybiera różne wartości z różnymi **prawdopodobieństwami**.

Interesuje nas jeden szczególny przypadek mianowicie, zmienna której zbiorem wartości
jest zbiór liczb rzeczywistych.

Rozkład zmiennej losowej: przyporządkowanie każdemu przedziałowi $(a, b)$ prawdopodobieństwa, że zmienna losowa przyjmie wartość z tego przedziału $P(a ≤ X ≤ b)$;
...
* wartość oczekiwana (eee pomyśl średnia)

* wariancja  (eee pomyśl wariancja, miara zmienności rozkładu)

Rozkład normalny:

Zmienna ma rozkład normalny o parametrach $m$, $\sigma$ co zapisujemy
jako $X \sim N(m, \sigma)$ jeżeli jej **funkcja gęstości** określona jest
wzorem:

$$f(x) = \frac{1}{\sigma \sqrt{2\pi }} \exp^{- (x - m)^2/ (2\sigma^2)}$$

![](./Standard_deviation_diagram.png)

* całkowita powierzchnia pod krzywą gęstości jest równa jeden

* pole pod krzywą dla przedziału $(a, b)$ to prawdopodobieństwo przyjęcia wartości z przedziału $(a, b)$

* pole pod krzywą od $-\infty$ do a, to wartość dystrybuanty $F(a)$

* reguła 3 sigm ($3 \sigma$)


![](./GretlNarzedzia.png)

Zadanie: jakie jest prawdopodobieństwo przyjęcia przez zmienną $N(3, 1)$ wartości
z przedziału $(3,4)$.


## Wnioskowanie statystyczne

Zmienna X ma określony rozkład 

Losujemy $n$-elementową próbę, otrzymując ciąg $x_1$, $x_2$, ... $x_n$. Gdybyśmy powtórzyli
losowanie zapewne otrzymamy inne wartości. Można traktować różne wartości $x_1$ jako realizacje
zmiennej $X_1$, różne wartości $x_2$ jako realizacje zmiennej $X_2$ itd...

Wnioskowanie o parametrach populacji generalnej na podstawie próby opiera się na 
funkcjach zmiennych losowych $X_1$, ... $X_n$ zwanych **statystykami z próby**

Przykładem statystyki z próby jest średnia z próby: $\bar X =  \frac{1}{n}\sum_{i=1}^n x_i$


Estymatorem $T$ nieznanego parametru $\phi$  nazywamy statystykę $T=f(X_1,...X_n)$, której
używa się do  estymacji tego parametru

Wartość estymatora dla konkretnej próby  ($x_1, ..., x_n$) nazywa się oceną.

Estymator $T$ jest **nieobciążony** jeżeli $E(T) = \phi$, przy wielokrotnym
losowaniu próby średnia z wartości przyjmowanych przez estymator jest równa  prawdziwej wartości
szacowanego parametru.

Estymator $T$ jest **zgodny** jeżeli w miarę wzrostu liczebności próby jest coraz większe
prawdopodobieństwo, że estymator będzie przyjmować wartości coraz bliższe prawdziwej wartości
parametru (prawdopodobieństwo dużego błędu maleje wraz ze wzrostem liczebności próby). 

Estymatorem $T_1$ jest **efektywniejszy** od $T_2$ jeżeli ma mniejszą wariancję.

Estymacja punktowa: na podstawie $n$-elementowej próby obliczamy ocenę parametru. Koniec.

### Weryfikacja hipotez parametrycznych 

Większość analiz statystycznych polega na porównaniu. W wyniku
tego porównania otrzymujemy liczbę. 

Załóżmy, że chcemy ustalić czy średni wiek w dwóch grupach ludzi jest identyczny.
Pobieramy próby i okazuje się że średnie wynoszą $\bar x_1 = 46,1$ oraz
$\bar x_2 = 47,2$. Różnica wynosi 1,1 lat i może być spowodowana błędem przypadkowym
(tj. gdybyśmy wylosowali jeszcze raz dwie próby, to wynik byłby zupełnie odmienny np 46,9 vs 46,5)
i/lub wynikać z tego, że faktycznie ludzie w drugiej grupie byli starsi. 
...
Zapisuje się to następująco ($m_{1}$/$m_{2}$ oznacza odpowiednio średnie 
w pierwszej i drugiej grupie):

$H_0$: różnica średnich wieku wynosi zero ($m_{1} = m_{2}$)

$H_1$: różnica średnich wieku jest różna od zera ($m_{1} \not= m_{2}$)
...
od wartości testowanych parametrów (w tym przypadku $m_{1}$ oraz $m_{2}$).
...
$u$ jest określany jako składnik losowy (zmienna losowa)
...
Wielkość $\phi^2 = RSK/OSK$ nazywamy **współczynnikiem zbieżności**.
...
![](./Wages-model-1.png){width=80%}
![](./Wages-model-2.png){width=80%}
...
jest statystycznie istotna. $R^2$ modelu wynosi 97,2%. 
